...
pub fn main() !void {
    ...
    const part_two = try sumCards(data);
...
fn sumCards(doc: []const u8) !usize {
    var stream = std.io.fixedBufferStream(doc);
    var buf: [1024]u8 = undefined;
    var index: usize = 1;
    var arena = std.heap.ArenaAllocator.init(std.heap.page_allocator);
    defer arena.deinit();
    var cards = std.ArrayList(SCards).init(arena.allocator());
    defer cards.deinit();
    while (true) {
        ...
        if (line_maybe) |line| {
            ...
            if (cards.items.len == 0) {
                ...
                for (cards.items) |card| {
                    if (index <= (card.cards_Num + card.match)) {
                        copies += 1;
                    }
                }
                try cards.appendNTimes(.{
                    .match = size,
                    .cards_Num = index,
                }, copies + 1);
            }
...
test "part two example" {
    const doc =
        ...
        \\Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11
    ;
    try std.testing.expect(try sumCards(doc) == 30);
}
